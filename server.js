...
function checkVictory(id) {
  ...
  io.emit('turn', turn, currentTurnPlayer ? currentTurnPlayer.name : '');
  }
}
...
io.on('connection', (socket) => {
  socket.on('registerName', (name) => {
    ...
    if (players.length >= 3 && !gameStarted) {
      ...
      const turnPlayerAlt = players.find(p => p.id === turn);
  io.emit('turn', turn, turnPlayerAlt ? turnPlayerAlt.name : '');
    }
  });
